...
class SiteDict
{
	...
	{
		if(!App::getInstance()->cache) Error::info('SiteDict Cache Error','Please load Cache!');
		$cache_key = 'sitedict_'.$dictname;
		$$dictname = App::getInstance()->cache->get($cache_key);
		if(empty($$dictname))
		{
			...
			require($data_file);
			App::getInstance()->cache->set($cache_key,$$dictname,self::$cache_life);
		}
		return $$dictname;
	}
	/**
	...
	{
		$cache_key = 'sitedict_'.$dictname;
		App::getInstance()->cache->delete($cache_key);
	}
}
